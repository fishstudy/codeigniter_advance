...
class Rest_client {
	...
	protected $args;
	protected $result;
	protected $content_charset_received;
	...
	function __construct($server_url=NULL){
    	...
    	//$this->curl_opt['VERBOSE'] = true;
		$this->curl_opt['HEADER'] = false;
		$this->curl_opt['FRESH_CONNECT'] = true;
		$this->curl_opt['SSL_VERIFYHOST'] = 2;
	//	$this->curl_opt['SSL_VERIFYPEER'] = TRUE;
		$this->curl_opt['USERAGENT'] = 'curl/7.21.3 (i686-pc-linux-gnu) libcurl/7.21.3 OpenSSL/0.9.8o zlib/1.2.3.4';
	}
	...
	/**
	 * Execute the request with HTTPS GET request method
	 * @return Rest_client
	 */
	public function get(){
		if($this->args!=NULL)
		...
		$this->result = curl_setopt_array($ch, $arr);
		...
		$temp  = 	explode(';', curl_getinfo($ch, CURLINFO_CONTENT_TYPE));
        if (!isset($temp[1])) $temp[1] = '*';
		list($this->content_type_received, $this->content_charset_received) = $temp;
	...
	/**
	 * Execute the request with HTTPS upload request method
	 * @return Rest_client
	 */
	public function upload($param=null){
		...
		$this->header_code_received = curl_getinfo($ch, CURLINFO_HTTP_CODE);
		list($this->content_type_received,$this->content_charset_received) = 
			explode(';', curl_getinfo($ch, CURLINFO_CONTENT_TYPE));
		curl_close($ch);
		return $this;
	}

	/**
	 * Execute the request with HTTPS POST request method
	 * @return Rest_client
	 */
	public function post(){
		...
		//set HTTP auth username and password is found
        if(isset($this->auth_user) && isset($this->auth_pwd) && $this->auth_user !=''){
            $arr[CURLOPT_USERPWD] = $this->auth_user .':'. $this->auth_pwd;
        }
	...
	/**
	 * Execute the request with HTTPS PUT request method
	 * @return Rest_client
	 */
	public function put(){
		$ch = curl_init( $this->server_url );
	...
	/**
	 * Execute the request with HTTPS DELETE request method
	 * @return Rest_client
	 */
	public function delete(){
		...
		$this->header_code_received = curl_getinfo($ch, CURLINFO_HTTP_CODE);
		list($this->content_type_received, $this->content_charset_received) = explode(';', curl_getinfo($ch, CURLINFO_CONTENT_TYPE));
	...
	public function resultContentCharset(){
		return $this->content_charset_received;
	}
	/**
	...
	public function xml_result($domObject=FALSE){
		if($domObject){
			...
			return simplexml_load_string($this->result, 'SimpleXMLElement', LIBXML_NOCDATA);
	}
	...
	public function proxy($param = null){
		$proxy_fields = array('PROXY'=>1, 'PROXYUSERPWD'=>1, 'PROXYAUTH'=>1,
				'PROXYPORT'=>1, 'PROXYTYPE'=>1);
		foreach ($param as $key=>$val){
			if (isset($proxy_fields[$key]))
