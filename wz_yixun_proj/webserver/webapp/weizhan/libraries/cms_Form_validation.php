...
class cms_Form_validation extends CI_Form_validation {
    ...
    {
        log_message('debug', 'my_Form_validation Initialized');
    ...
    function _parse_field($str){
        list($c, $table, $column) = explode('[', $str);
        $c      = trim($c, ']'); 
        $table  = trim($table, ']'); 
        $column = trim($column, ']'); 
        return array($c, $table, $column);
    }
    function _is_file_field($rules){
        foreach($rules as $rule){
            if (strncmp($rule, 'file_', strlen('file_')) == 0)
                return TRUE;
        }
        return FALSE;
    }
    function _execute($row, $rules, $postdata = NULL, $cycles = 0)
    {
        ...
        list($c, $table, $column) = $this->_parse_field($row['field']);
        if ($this->_is_file_field($rules)){
            if(isset($_FILES[$c]['name'][$table][$column]))
            {// it is a file so process as a file
                log_message('debug','processing as a file');
                foreach(array('name', 'type', 'tmp_name', 'error', 'size') as $v){
                    $postdata[$v] = $_FILES[$c][$v][$table][$column];
                }
                ...
                $this->_error_array[$row['field']] = $this->file_upload_error_message(UPLOAD_ERR_NO_FILE);
                return FALSE;
            }
        }
        else
    ...
    function file_size_min($file,$min_size)
    {
        ...
        {
            $this->set_message('file_size_min',"%s is too small. (Min allowed is $min_size)");
            return FALSE;
        }
        return true;
    }    
    ...
    function file_allowed_type($file,$type)
    {
        ...
        $ext_groups['pdf_document'] = array('pdf');
        $ext_groups['compressed'] = array('zip','gzip','tar','gz');
        $ext_groups['text'] = array('csv','txt');
     ...
     * set error msg
     */
    function set_error_array($key, $msg){
        
        return $this->_error_array[$key] = $msg;
    }

	/**
     * clean error msg
     */
    function clean_error_array(){
        return $this->_error_array = array();
    }
}
