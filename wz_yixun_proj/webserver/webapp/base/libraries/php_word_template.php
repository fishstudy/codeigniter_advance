...
class Php_word_template
{
     ...
     * @access   public
     */
    function Php_word_template($root = '', $options = null)
    {
        if (!is_null($options)) {
    ...
    function save($saveFileName){
        //$this->zip = new ZipArchive();
        $this->zip->open($this->tempFileName);
        $this->zip->addFromString('word/document.xml', $this->get());
        $this->zip->close();
        return rename($this->tempFileName, $saveFileName);
    }
    /**
    ...
    function loadTemplatefile( $filename,
        ...
        $this->tempFileName = sprintf('%s.docx', tempnam("/tmp", 'hunter')); //$path.DIRECTORY_SEPARATOR.time().'.docx';

        copy($this->fileRoot. '/'. $filename, $this->tempFileName); // Copy the source File to the temp File
        $template = '';
        if (!$this->flagCacheTemplatefile
                || $this->lastTemplatefile != $this->tempFileName
           ) {
