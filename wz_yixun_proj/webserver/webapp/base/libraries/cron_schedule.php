...
class Cron_schedule
{
    ...
    protected $cron_schedule_table_name = 'weidian.schedules';
    ...
    protected $CI;
    protected $db;
    protected $_cron_last_schedule_generate;
    protected $_cron_last_history_cleanup;
     ...
     * @param $param
     */
    function onetime($class,$function,$param=array()) {
    	$filepath = APPPATH.$param['path'];
        require($filepath);
        $SCHEDULE = new $class;
    ...
    public function dispatch()
    {
        ...
        $now = time();
        /*$jobConfig = $this->cron_schedule['suggestion_all'];
        ...
        foreach ($schedules as $schedule) {
            $jobConfig = empty($this->cron_schedule[$schedule['job_code']]) ? array() : $this->cron_schedule[$schedule['job_code']];
            if (!$jobConfig || !$jobConfig['run']) {
                continue;
            }

            $runConfig = $jobConfig['run'];
            //print_r($runConfig);die();
            ...
            try {
                ...
                if ( ! file_exists($filepath)) {
                    throw new Exception(sprintf('No cron schedule file found. filepath:%s,', $filepath));
                }
                ...
                $result = $this->db->where('id', $schedule['id'])
                ...
                $this->updateSchedule($schedule['id'], array(
    ...
    public function generate()
    {
         ...
         */
        $cache_res = $this->CI->load->driver('cache');
        log_message('debug', 'load cache res:'.$cache_res);
        if($cache_res){
	        $lastRun = $this->CI->cache->file->get(self::CACHE_KEY_LAST_SCHEDULE_GENERATE_AT);
	        $this->_log->debug("lastRun:".$lastRun);
    ...
    public function cleanup()
    {
        ...
        $history = $this->db->where_in('status', array(
                self::STATUS_SUCCESS,
                self::STATUS_MISSED,
                self::STATUS_ERROR,
            ))->get($this->cron_schedule_table_name)
    ...
    {
        return $this->db->insert($this->cron_schedule_table_name, $schedule);
    }

    public function updateSchedule($id, $schedule_data)
    {
