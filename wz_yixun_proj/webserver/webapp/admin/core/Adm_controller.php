...
class Adm_controller extends CI_Controller {
    ...
    /**
     * 不需要登陆跳转的action数组,其余的都跳转到login
     * 
     * @var array
     * @access protected
     */
    protected $redirect_action = array('master_login');
    ...
    function __construct() {
        ...
        $this->_response['admindomain'] =  $this->config->item('admindomain');
        $this->_response['admin_static_domain'] =  $this->config->item('admin_static_domain');
    ...
    function is_login( $format = ENVIRONMENT ){
    	...
    	if ($format == ENVIRONMENT) {
    		//先简单化,以后对接admin系统的用户中心。
    		if ( isset($_COOKIE['wzadminuid']) && $_COOKIE['wzadminuid']==10000 && isset($_COOKIE['wzadminname']) && $_COOKIE['wzadminname']='admin' ) {
    			$this->uid = $_COOKIE['wzadminuid'];
    			$this->user_info = array('uid'=> $_COOKIE['wzadminuid'],'adminname'=>$_COOKIE['wzadminname']);
    			$result = true;
    		}else {
    			$result = false;
    		}
     ...
     * @return mixed
     */
    public function nologin_redirect() {
    	$domainName = $this->config->item('admindomain');
...
//     	$url = $protocol .'://base.'.$domainName.'/'.$redirectUrl.'?url=' . urlencode($currentUrl);
    	$url = $domainName . "admin/master/login";
    	header("location: " . $url);
    	return ;
    }
