...
class Memcached_library
{
	...
	public function __construct($cname = '')
	{
        ...
        if ($_API_APP_START_PATH) {
            $_APPPATH = $_API_APP_START_PATH;
        }

		$cachename = $cname ? 'memcached_' . $cname : 'memcached';
		...
		if($this->client_type) 
		{
	        if ( ! defined('ENVIRONMENT') OR ! file_exists($file_path = $_APPPATH.'config/'.ENVIRONMENT.'/'.$cachename.'.php'))
	        {
	            if ( ! file_exists($file_path = $_APPPATH.'config/'.$cachename.'.php'))
	            {
	                show_error('The configuration file '.$cachename.'.php does not exist.');
	            }
	        }
        	include($file_path);
        	
        	$this->config = $config[$cachename];
